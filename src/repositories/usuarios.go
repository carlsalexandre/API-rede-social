...
func (repositorie usuarios) Criar(usuario models.Usuario) (uint64, error) {
	statement, erro := repositorie.db.Prepare("insert into usuarios (nome, nick, email, senha) values(?,?,?,?)")
	if erro != nil {
		return 0, erro
	}
...
func (respositorio usuarios) Seguir(usuarioID, seguidorID uint64) error {
	statement, erro := respositorio.db.Prepare("insert ignore into seguidores (usuario_id, seguidor_id) values (?, ?)")
	if erro != nil {
		return erro 
	}
	defer statement.Close()
